...
xhr.onreadystatechange = function(){
    if(xhr.readyState == 4 && xhr.status == 200){
        ...
        for(var i = 0; i < json.data.length; i++){
            ...
            td3.appendChild(document.createTextNode(json.data[i].year));

            td4.appendChild(document.createTextNode(json.data[i].color));
            td4.setAttribute("style", `color: ${json.data[i].color};`);
